...
def read_cifar10(data_dir, is_train, batch_size, shuffle):
    ...
    img_height = 32
    img_depth = 3
    label_bytes = 1  #label位元數
    image_bytes = img_width*img_height*img_depth
    
    
    with tf.name_scope('input'):
        
        if is_train: # Get training data
            filenames = [os.path.join(data_dir, 'data_batch_%d.bin' %ii)
                                        for ii in np.arange(1, 6)]
        else:        # Get test data
            filenames = [os.path.join(data_dir, 'test_batch.bin')]
          
        filename_queue = tf.train.string_input_producer(filenames)#圖像分開使用 slice_input_producer
        ...
        label = tf.slice(record_bytes, [0], [label_bytes])# 切出第一個位元(label)
        label = tf.cast(label, tf.int32) # convert to int32
        
        image_raw = tf.slice(record_bytes, [label_bytes], [image_bytes]) # 1~32x32
...
#        image = tf.image.random_contrast(image,lower=0.2,upper=1.8)
        ...
        image = tf.image.per_image_standardization(image) #圖像標準化0~255 => 0~1 
...
#
#data_dir = 'C:/Users/aup67/ML/CIFAR10/data/cifar-10-batches-bin'
#BATCH_SIZE = 10
...
#                print('label: %d' %np.argmax(label[j]))
#                plt.imshow(img[j,:,:,:])
#                plt.show()
#            i+=1
#            
